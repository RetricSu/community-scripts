...
export class NPMSDKDiscoverer {
  async discoverSDKs(): Promise<string[]> {
    ...
    for (const pattern of patterns) {
      try {
        ...
        console.log(`Found ${packages.length} packages matching "${pattern}"`);
      } catch (error) {
        const errorMessage =
    ...
    console.log(`📦 Total unique SDK packages found: ${uniquePackages.length}`);
  ...
  async getPackageInfo(packageName: string): Promise<NPMPackage | null> {
    try {
      const { stdout } = await execAsync(`pnpm view ${packageName} --json`);
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      console.warn(`Failed to get info for ${packageName}: ${errorMessage}`);
      return null;
    }
  }
